import React from "react";
import "./Team.css";
import webmaster from "../../assets/webmaster.png";
import kamalesh from "../../assets/Kamalesh.jpg";
import VC from "../../assets/vice.jpg";
import aathithya from "../../assets/aathithya_event_management.jpg";
import chairman from "../../assets/chairman.jpg";
import deepthi from "../../assets/Deepthi.jpeg";

const Team = () => {
  return (
    <div className="team-page">
      <h1 >OFFICE BEARERS</h1>
      <div className="team">
        <div className="card1">
          <img
            src={chairman}
            alt="BHUVAANESH R"
          />
          <div>
            <h2>Bhuvaanesh R</h2>
            <h3>CHAIRMAN</h3>
            <p></p>
            <a target="_blank" href="https://www.linkedin.com/in/bhuvaanesh-r">Follow Account</a>
          </div>
        </div>
        <div className="card1">
          <img
            src={VC}
            alt="JODERICK SHERWIN"
          />
          <div>
            <h2>Joderick Sherwin</h2>
            <h3>VICE CHAIRMAN</h3>
            <p></p>
            <a target="_blank" href="https://www.linkedin.com/in/joderick-sherwin-j/">Follow Account</a>
          </div>
        </div>
        <div className="card1">
          <img
            src="https://upload.wikimedia.org/wikipedia/commons/7/7c/Profile_avatar_placeholder_large.png?20150327203541"
            alt="DEEPIGA DHARSHINI C"
          />
          <div>
            <h2>Deepiga Dharshini C</h2>
            <h3>SECRETARY & TREASURER</h3>
            <p></p>
            <a target="_blank" href="https://www.linkedin.com/in/deepiga-dharshini-ts20">Follow Account</a>
          </div>
        </div>
        <div className="card1">
          <img
            src={webmaster}
            alt="BHARATH KUMAR L"
          />
          <div>
            <h2>Bharath Kumar L</h2>
            <h3>WEBMASTER</h3>
            <p></p>
            <a target="_blank" href="https://www.linkedin.com/in/bharath-kumar-l-955304289/">Follow Account</a>
          </div>
        </div>
      </div>
      <h1 className="my-4">DOMAIN LEADS</h1>
      <div className="team" id="dl">
        <div className="card1">
          <img
            src="https://upload.wikimedia.org/wikipedia/commons/7/7c/Profile_avatar_placeholder_large.png?20150327203541"
            alt="ROSHINI D"
          />
          <div >
            <h2>Roshini D</h2>
            <h3>WEB DEVELOPMENT</h3>
            <p></p>
            <a target="_blank" href="#">Follow Account</a>
          </div>
        </div>
        <div className="card1">
          <img
            src={VC}
            alt="Joderick Sherwin"
          />
          <div>
            <h2>Joderick Sherwin</h2>
            <h3>MACHINE LEARNING</h3>
            <p></p>
            <a target="_blank" href="https://www.linkedin.com/in/joderick-sherwin-j/">Follow Account</a>
          </div>
        </div>
        <div className="card1">
          <img
            src="https://upload.wikimedia.org/wikipedia/commons/7/7c/Profile_avatar_placeholder_large.png?20150327203541"
            alt="Amrutha B J"
          />
          <div>
            <h2>Amrutha B J</h2>
            <h3>APP DEVELOPMENT</h3>
            <p></p>
            <a target="_blank" href="#">Follow Account</a>
          </div>
        </div>
        <div className="card1">
          <img
            src="https://upload.wikimedia.org/wikipedia/commons/7/7c/Profile_avatar_placeholder_large.png?20150327203541"
            alt="Caroline Suja J S"
          />
          <div>
            <h2>Caroline Suja J S</h2>
            <h3>MEDIA MANAGEMENT</h3>
            <p></p>
            <a target="_blank" href="#">Follow Account</a>
          </div>
        </div>
        <div className="card1">
          <img
            src="https://upload.wikimedia.org/wikipedia/commons/7/7c/Profile_avatar_placeholder_large.png?20150327203541"
            alt="Ragul Sowmiyanarayanan G K"
          />
          <div>
            <h2>Ragul Sowmiyanarayanan G K</h2>
            <h3>EVENT MANAGEMENT</h3>
            <p></p>
            <a target="_blank" href="#">Follow Account</a>
          </div>
        </div>
        <div className="card1">
          <img
            src="https://upload.wikimedia.org/wikipedia/commons/7/7c/Profile_avatar_placeholder_large.png?20150327203541"
            alt="Nandha Kumar P"
          />
          <div>
            <h2>Nandha Kumar P</h2>
            <h3>DESIGN TEAM</h3>
            <p></p>
            <a target="_blank" href="#">Follow Account</a>
          </div>
        </div>
      </div>
      <h1 className="my-4">CORE TEAM</h1>
      <div className="team dl" id="ct">
        <div className="card1">
          <img
            src="https://upload.wikimedia.org/wikipedia/commons/7/7c/Profile_avatar_placeholder_large.png?20150327203541"
            alt="Nithish Rao P"
          />
          <div>
            <h2>Nithish Rao P</h2>
            <h3>WEB DEVELOPMENT</h3>
            <p></p>
            <a target="_blank" href="#">Follow Account</a>
          </div>
        </div>
        <div className="card1">
          <img
            src="https://upload.wikimedia.org/wikipedia/commons/7/7c/Profile_avatar_placeholder_large.png?20150327203541"
            alt="Varun G"
          />
          <div>
            <h2>Varun G</h2>
            <h3>WEB DEVELOPMENT</h3>
            <p></p>
            <a target="_blank" href="#">Follow Account</a>
          </div>
        </div>
        <div className="card1">
          <img
            src="https://upload.wikimedia.org/wikipedia/commons/7/7c/Profile_avatar_placeholder_large.png?20150327203541"
            alt="Suranjana S"
          />
          <div>
            <h2>Suranjana S</h2>
            <h3>MACHINE LEARNING</h3>
            <p></p>
            <a target="_blank" href="#">Follow Account</a>
          </div>
        </div>
        <div className="card1">
          <img
            src="https://upload.wikimedia.org/wikipedia/commons/7/7c/Profile_avatar_placeholder_large.png?20150327203541"
            alt="Sri Harshavardhanan R"
          />
          <div>
            <h2>Sri Harshavardhanan R</h2>
            <h3>APP DEVELOPMENT</h3>
            <p></p>
            <a target="_blank" href="#">Follow Account</a>
          </div>
        </div>
        <div className="card1">
          <img
            src={deepthi}
            alt="Deepthi P"
          />
          <div>
            <h2>Deepthi P</h2>
            <h3>CONTENT WRITER</h3>
            <p></p>
            <a target="_blank" href="https://www.linkedin.com/in/deepthi-pavendan-b48853257/">Follow Account</a>
          </div>
        </div>
        <div className="card1">
          <img
            src="https://upload.wikimedia.org/wikipedia/commons/7/7c/Profile_avatar_placeholder_large.png?20150327203541"
            alt="Dhanasree L P"
          />
          <div>
            <h2>Dhanasree L P</h2>
            <h3>EVENT MANAGER</h3>
            <p></p>
            <a target="_blank" href="#">Follow Account</a>
          </div>
        </div>
        <div className="card1">
          <img
            src="https://upload.wikimedia.org/wikipedia/commons/7/7c/Profile_avatar_placeholder_large.png?20150327203541"
            alt="Bharath D"
          />
          <div>
            <h2>Bharath D</h2>
            <h3>EVENT MANAGEMENT</h3>
            <p></p>
            <a target="_blank" href="https://www.linkedin.com/in/bharath-d-2a60ab2a6?utm_source=share&utm_campaign=share_via&utm_content=profile&utm_medium=android_app">Follow Account</a>
          </div>
        </div>
        <div className="card1">
          <img
            src="https://upload.wikimedia.org/wikipedia/commons/7/7c/Profile_avatar_placeholder_large.png?20150327203541"
            alt="AgishRaj R"
          />
          <div>
            <h2>AgishRaj R</h2>
            <h3>EVENT MANAGEMENT</h3>
            <p></p>
            <a target="_blank" href="#">Follow Account</a>
          </div>
        </div>
        <div className="card1">
          <img
            src="https://upload.wikimedia.org/wikipedia/commons/7/7c/Profile_avatar_placeholder_large.png?20150327203541"
            alt="Bharath B"
          />
          <div>
            <h2>Bharath B</h2>
            <h3>EVENT MANAGEMENT</h3>
            <p></p>
            <a target="_blank" href="#">Follow Account</a>
          </div>
        </div>
        <div className="card1">
          <img
            src="https://upload.wikimedia.org/wikipedia/commons/7/7c/Profile_avatar_placeholder_large.png?20150327203541"
            alt="Swathi G"
          />
          <div>
            <h2>Swathi G</h2>
            <h3>EVENT MANAGEMENT</h3>
            <p></p>
            <a target="_blank" href="#">Follow Account</a>
          </div>
        </div>
        <div className="card1">
          <img
            src={aathithya}
            alt="Aathithya S K"
          />
          <div>
            <h2>Aathithya S K</h2>
            <h3>EVENT MANAGEMENT</h3>
            <p></p>
            <a target="_blank" href="https://www.linkedin.com/in/athithya-s-k/">Follow Account</a>
          </div>
        </div>
        <div className="card1">
          <img
            src={kamalesh}
            alt="Kamalesh S P"
          />
          <div>
            <h2>Kamalesh S P</h2>
            <h3>EVENT MANAGEMENT</h3>
            <p></p>
            <a target="_blank" href="#">Follow Account</a>
          </div>
        </div>
        <div className="card1">
          <img
            src="https://upload.wikimedia.org/wikipedia/commons/7/7c/Profile_avatar_placeholder_large.png?20150327203541"
            alt="Dinisha R"
          />
          <div>
            <h2>Dinisha R</h2>
            <h3>EVENT MANAGEMENT</h3>
            <p></p>
            <a target="_blank" href="#">Follow Account</a>
          </div>
        </div>
        <div className="card1">
          <img
            src="https://upload.wikimedia.org/wikipedia/commons/7/7c/Profile_avatar_placeholder_large.png?20150327203541"
            alt="Divya K"
          />
          <div>
            <h2>Divya K</h2>
            <h3>EVENT MANAGEMENT</h3>
            <p></p>
            <a target="_blank" href="https://www.linkedin.com/in/divya-k-3aa1862a2/">Follow Account</a>
          </div>
        </div>
        <div className="card1">
          <img
            src="https://upload.wikimedia.org/wikipedia/commons/7/7c/Profile_avatar_placeholder_large.png?20150327203541"
            alt="Vishwa J"
          />
          <div>
            <h2>Vishwa J</h2>
            <h3>EVENT MANAGEMENT</h3>
            <p></p>
            <a target="_blank" href="#">Follow Account</a>
          </div>
        </div>
        <div className="card1">
          <img
            src="https://upload.wikimedia.org/wikipedia/commons/7/7c/Profile_avatar_placeholder_large.png?20150327203541"
            alt="Ramalingam S"
          />
          <div>
            <h2>Ramalingam S</h2>
            <h3>EVENT MANAGEMENT</h3>
            <p></p>
            <a target="_blank" href="#">Follow Account</a>
          </div>
        </div>
        <div className="card1">
          <img
            src="https://upload.wikimedia.org/wikipedia/commons/7/7c/Profile_avatar_placeholder_large.png?20150327203541"
            alt="Bhuvaneshwari M"
          />
          <div>
            <h2>Bhuvaneshwari M</h2>
            <h3>EVENT MANAGEMENT - PR TEAM</h3>
            <p></p>
            <a target="_blank" href="#">Follow Account</a>
          </div>
        </div>
        <div className="card1">
          <img
            src="https://upload.wikimedia.org/wikipedia/commons/7/7c/Profile_avatar_placeholder_large.png?20150327203541"
            alt="Himeshwar N"
          />
          <div>
            <h2>Himeshwar N</h2>
            <h3>EVENT MANAGEMENT - PR TEAM</h3>
            <p></p>
            <a target="_blank" href="#">Follow Account</a>
          </div>
        </div>
        <div className="card1">
          <img
            src="https://upload.wikimedia.org/wikipedia/commons/7/7c/Profile_avatar_placeholder_large.png?20150327203541"
            alt="Jayadharsini M"
          />
          <div>
            <h2>Jayadharsini M</h2>
            <h3>EVENT MANAGEMENT - PR TEAM</h3>
            <p></p>
            <a target="_blank" href="https://www.linkedin.com/in/jayadharsini-mathiyalagan-72607b2a2?utm_source=share&utm_campaign=share_via&utm_content=profile&utm_medium=android_app">Follow Account</a>
          </div>
        </div>
        <div className="card1">
          <img
            src="https://upload.wikimedia.org/wikipedia/commons/7/7c/Profile_avatar_placeholder_large.png?20150327203541"
            alt="Gokula Sarathy PS"
          />
          <div>
            <h2>Gokula Sarathy PS</h2>
            <h3>DESIGN TEAM</h3>
            <p></p>
            <a target="_blank" href="https://www.linkedin.com/in/gokula-sarathy-p-s-8343121b8?utm_source=share&utm_campaign=share_via&utm_content=profile&utm_medium=android_app">Follow Account</a>
          </div>
        </div>
        <div className="card1">
          <img
            src="https://upload.wikimedia.org/wikipedia/commons/7/7c/Profile_avatar_placeholder_large.png?20150327203541"
            alt="Farheen Tabassum H"
          />
          <div>
            <h2>Farheen Tabassum H</h2>
            <h3>DESIGN TEAM</h3>
            <p></p>
            <a target="_blank" href="#">Follow Account</a>
          </div>
        </div>
        <div className="card1">
          <img
            src="https://upload.wikimedia.org/wikipedia/commons/7/7c/Profile_avatar_placeholder_large.png?20150327203541"
            alt="Srivarshni S"
          />
          <div>
            <h2>Srivarshni S</h2>
            <h3>DESIGN TEAM</h3>
            <p></p>
            <a target="_blank" href="#">Follow Account</a>
          </div>
        </div>
      </div>
    </div>
  );
};

export default Team;
